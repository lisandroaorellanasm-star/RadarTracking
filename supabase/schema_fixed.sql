...
-- ============================================
-- BUSINESSES TABLE (Create first to avoid circular dependency)
-- ============================================
...
-- Create spatial index for geolocation queries (FIXED)
CREATE INDEX idx_locations_geofence ON locations USING GIST(ST_SetSRID(ST_MakePoint(longitude, latitude), 4326));
...
BEGIN
    ...
    IF v_location.geofence_polygon IS NOT NULL THEN
        RETURN ST_Contains(v_location.geofence_polygon, ST_SetSRID(ST_MakePoint(p_longitude, p_latitude), 4326));
    END IF;
    
    -- Check circular geofence
    v_geofence_center := ST_SetSRID(ST_MakePoint(v_location.longitude, v_location.latitude), 4326)::geography;
